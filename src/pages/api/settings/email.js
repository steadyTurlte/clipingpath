...
const getEmailSettings = () => {
  try {
    ...
    if (settings.email) {
      ...
      return {
        adminEmail: settings.email.adminEmail || DEFAULT_EMAIL_SETTINGS.adminEmail
      };
    } else {
...
const saveEmailSettings = (settings) => {
  try {
    ...
    const existingEmailSettings = existingSettings.email || DEFAULT_EMAIL_SETTINGS;
    ...
    const success = saveData('settings', existingSettings);
...
export default function handler(req, res) {
  ...
  // Handle PUT request
  if (req.method === 'PUT') {
    const success = saveEmailSettings(req.body);
