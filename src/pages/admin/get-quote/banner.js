...
const GetQuoteBannerEditor = () => {
  ...
  useEffect(() => {
    const fetchBannerData = async () => {
      try {
        const response = await fetch('/api/content/get-quote?section=banner');
        if (response.ok) {
  ...
  return (
    <AdminLayout>
      <Head>
        <title>Edit Get A Quote Banner | Photodit Admin</title>
      </Head>

      <div className="admin-editor">
        <div className="admin-editor__header">
          <div className="admin-editor__title-wrapper">
            <Link href="/admin/get-quote" className="admin-editor__back-link">
              <i className="fa-solid fa-arrow-left"></i>
            </Link>
            <h1 className="admin-editor__title">Edit Get A Quote Banner</h1>
          </div>
...
export default GetQuoteBannerEditor;
